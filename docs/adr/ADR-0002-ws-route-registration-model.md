...
- `route.actor` может быть `{/this}` или явным именем другого актора (включая удаленного);
...
5. `route.actor` может указывать на другой actor по имени (в том числе на удаленный/межнодовый); для такого маршрута не требуется локальная регистрация функции этого актора.
6. Для WS actor placeholder `{/this}` в `actor` поддерживается как стандарт.
7. `mapping` валидируется при загрузке типа (fail-fast).
8. Дубликат route (одинаковые `method+pattern` внутри типа) приводит к фатальной ошибке загрузки типа.
9. Дубликат action-route (попытка повесить второй route на тот же action) приводит к фатальной ошибке загрузки типа.
10. В имени `route.actor` допустима подстановка параметров из `pattern` через `{...}`.
